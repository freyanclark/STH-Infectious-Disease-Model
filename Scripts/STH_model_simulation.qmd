...
names(params$sigma) <- paste0("IU", 1:n_IU)
...
Plot movement matrix ($\alpha$).
```{r}
# convert matrix to long format
alpha_df <- melt(params$movement_matrix)

colnames(alpha_df) <- c("Home_IU", "Visited_IU", "fraction")

ggplot(alpha_df, aes(x = Visited_IU, y = Home_IU, fill = fraction)) +
  geom_tile(color = "white") +
  geom_text(aes(label = round(fraction, 2)), size = 4) +
  scale_fill_gradient(low = "white", high = "#2C7FB8") +
  labs(
    x = "IU visited",
    y = "Home IU",
    fill = "Fraction\nof time",
    title = expression("Movement matrix, (" * alpha * ")")
  ) +
  theme_minimal()
```
...
Update demography.
```{r}
update_demography <- function(pop, W, params, demography = TRUE){
  
  # if demography disabled, return unchanged
  if(!demography){
    return(list(pop = pop, W = W))
  }
...
update_worms <- function(pop, W, E, alpha, params){
  ...
  sigma_vec <- params$sigma[pop$IU_home]
  
  sigma_mat <- matrix(
    rep(sigma_vec, length(species)),
    nrow = n,
    ncol = length(species)
  )
...
Single time step.
```{r}
step_model <- function(state, params, t, treatment_schedule, demography = TRUE){
  
  # 1. demography
  demog <- update_demography(state$pop, state$W, params, demography)
  pop <- demog$pop
  W   <- demog$W
...
equilibrium_prevalence <- function(params,
  ...
  for(t in 1:T_eq){ 
    state <- step_model(state, params, t, treatment_schedule = NULL, demography = FALSE) 
  }
...
calibrate_sigma <- function(params,
  ...
  for(iu in 1:params$n_IU){
    ...
    obj_fn <- function(log_sigma){ # log(sigma) so sigma > 0
      ...
      # sum of squared relative errors across all species 
      # (scaled by target prevalence to prevent higher prev species from dominating)
      sum(((prev_model - prev_target) / prev_target)^2)
    }
    ...
    # store optimal sigma for current IU
...
# check population size stays the same
sum(pop$alive) # start
sum(state_eq$pop$alive) # end
```

## Baseline simulation
```{r}
T  <- 30*(1/dt) # number of time steps (30 years)

# pre-allocate list to store results
results <- vector("list", T)  # length = number of timesteps

# initial state
state <- list(pop = state_eq$pop, W = state_eq$W, E = state_eq$E) # initialise with equilibrium
...
species_all <- c(species, "anySTH")
n_species_all <- length(species_all)

## total SAC prevalence
prev_SAC <- sapply(results, function(state) {
  
  sac_idx <- state$pop$age_group %in% SAC_groups & state$pop$alive
  if (sum(sac_idx) == 0) return(rep(0, n_species_all))
  
  W_sac <- state$W[sac_idx, , drop = FALSE]
  
  # species prevalence
  prev_species <- colSums(W_sac > 0) / nrow(W_sac)
  
  # any STH prevalence
  anySTH <- rowSums(W_sac > 0) > 0
  prev_anySTH <- mean(anySTH)
  
  c(prev_species, anySTH = prev_anySTH)
})

### timesteps × species
prev_SAC <- t(prev_SAC)
colnames(prev_SAC) <- species_all
...
prev_SAC_IU <- lapply(results, function(state) {
  ...
  if (sum(sac_idx) == 0) {
    return(matrix(
      0, nrow = n_IU, ncol = n_species_all,
      dimnames = list(paste0("IU", 1:n_IU), species_all)
    ))
  }
  ...
  sapply(1:n_IU, function(iu) {
    ...
    if (sum(in_IU) == 0) return(rep(0, n_species_all))
    
    W_sub <- W_sac[in_IU, , drop = FALSE]
    
    prev_species <- colSums(W_sub > 0)/nrow(W_sub)
    
    anySTH <- rowSums(W_sub > 0) > 0
    prev_anySTH <- mean(anySTH)
...
Plot total population size for entire simulation.
```{r}
# total population size at each time step
pop_size <- sapply(results, function(state){
  sum(state$pop$alive)
})

df_pop <- data.frame(
  time = seq_along(pop_size) * dt,
  population = pop_size
)

ggplot(df_pop, aes(x = time, y = population)) +
...
Plot IU-level population size for entire simulation.
```{r}
# IU-level population size at each time step
pop_IU <- lapply(results, function(state){
  
  alive_pop <- state$pop[state$pop$alive, ]
  
  counts <- table(factor(alive_pop$IU_home, levels = 1:n_IU))
  
  data.frame(
    IU = paste0("IU", 1:n_IU),
    population = as.numeric(counts)
  )
})

pop_IU <- do.call(rbind, pop_IU)

pop_IU$time <- rep(seq_len(T) * dt, each = n_IU)

ggplot(pop_IU, aes(x = time, y = population)) +
  geom_line(color = "#2C7FB8", linewidth = 0.9) +
  facet_wrap(~ IU, ncol = 1) +
  labs(
    x = "Time (years)",
    y = "Population size",
    title = "Population size over time by IU"
  ) +
  scale_y_continuous(
    limits = c(0, 500)
  ) +
...
treatment_schedule <- data.frame(
  time = c(5/dt, 5/dt, 6/dt, 6/dt, 7/dt, 8/dt),  # time step of treatment
  IU   = c(rep(c(1, 2), 2), rep(1, 2)), # IU receiving treatment
  tau  = rep(0.75, 6), # coverage for SACs
  drug = rep("Albendazole", 6)
)
```
...
df_overall$species <- factor(df_overall$species, levels = names(species_cols))
